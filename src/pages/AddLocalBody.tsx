...
const AddLocalBody = () => {
  ...
  useEffect(() => {
    if (id) {
      const fetchApi = async () => {
        await api.get(`/master/localBody/${id}`)
          .then((res: any) => {
            if (res.data.success) {
              setLocalBody((prev) => ({ ...prev, ...res.data.data }));
            }
          });
      }
      fetchApi()
  ...
  const handleChange = (e: any) => {
    ...
    setLocalBody({ ...localBody, [name]: value });
  };
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      if (id) {
        const response = await api.put(`/master/localBody/${id}`, localBody);
        ...
        const response = await api.post("/master/localBody", localBody);
    ...
    <div className="container mt-4">
      <div className="card shadow-sm border-0">
        <div className="card-header bg-primary text-white py-2">
          <h6 className="m-0 fw-semibold">{`${id ? "Update" : "Add"} LocalBody`}</h6>
        </div>
